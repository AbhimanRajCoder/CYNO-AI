...
async def test_ollama():
    """Test if Ollama LLM is responding"""
    try:
        start = time.time()
        ...
        return {
            "status": "error",
            "error": str(e),
            "hint": "Run 'ollama serve' and ensure model is pulled with 'ollama pull llama3.2'"
        }
...
async def check_model():
    ...
    try:
        ...
        return {
            ...
            "hint": f"Run 'ollama pull {MODEL_NAME}' if model is not available"
        }
    except Exception as e:
        return {
            "status": "error",
